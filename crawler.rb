...
def create_page_record(list_restaurant)
  name = list_restaurant.text.strip
  restaurant_url = list_restaurant['href']
  return if Page.exists?(restaurant_url: restaurant_url)

  # restaurant_html = fetch_url(restaurant_url)
  Page.create(
    crawl_status: 1,
    name: name,
...
def crawl(url)
  ...
  loop do
    ...
    lists_restaurant = doc.css('div.list-rst__rst-name > a')
